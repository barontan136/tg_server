...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.tabMove = function(){
		$(window).on("resize",function(){
          ...
          //动态计算窗口的高度
        Tab.prototype.autoHeight = function(){
            //需要动态变动的div
//            var ah =  $('.layui-form');
//            var ifr = $('.layui-show').find('iframe').eq(0);
//            ifr.load(function(){  
//            //窗口高度
//            var wheight = $(window).height();
//            //父页面高度
//            var pdheight = $(document).height();
//            //当前iframe内容高度
//            var cdheight = ifr[0].contentWindow.document.body.scrollHeight;
//            //最大值
//            var maxh = (((wheight>pdheight)?wheight:pdheight)>cdheight ? ((wheight>pdheight)?wheight:pdheight) : cdheight);
//            ah.height(maxh)
//            ifr[0].height = maxh;
//           })
       }
	...
	$("body").on("click",".top_tab li",function(){
		...
		//element.tabChange(tabFilter,$(this).attr("lay-id")).init();
		 //左侧菜单高亮
                $('.navBar').find('.layui-this').removeClass('layui-this');
                var _this = $(this)
                $(this).addClass('layui-anim layui-anim-scale');
                var cur = $('.navBar').find('a[data-url*="'+JSON.parse(window.sessionStorage.getItem("curmenu")).href+'"]');
                cur.parent().addClass('layui-this').addClass('layui-anim layui-anim-scale');
                cur.closest('li').addClass('layui-nav-itemed');
                setTimeout(function(){
                     _this.removeClass('layui-anim layui-anim-scale');
                     cur.parent().removeClass('layui-anim layui-anim-scale');
                },300)
	})
