...
(function () {
    ...
    function setTabFocus(id) {
        ...
        switch (id) {
            ...
            case 'online1':
                setAlign(editor.getOpt('imageManagerInsertAlign'));
                onlineImage1 = onlineImage1 || new OnlineImage1('imageList1');
                onlineImage1.reset();
                break;
            case 'search':
                setAlign(editor.getOpt('imageManagerInsertAlign'));
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                ...
                case 'online':
                    ...
                    break;
                case 'online1':
                    list = onlineImage1.getInsertList();
                    break;
                case 'search':
    ...
    /****监听图片分组**************/
    $('body').on('click','[data-tagimg-id]',function(){
        $('[data-tagimg-id]').removeClass('checkimg');
        $(this).addClass('checkimg');
        onlineImage = onlineImage || new OnlineImage('imageList');
        onlineImage.initContainer();
        onlineImage.initEvents();
        var list = [];
        var id = $(this).attr('data-tagimg-id');
        if(id =="全部图片"){
            onlineImage.pushData(onlineimagelist);
        }else{
         $.each(onlineimagelist,function(){
            if(this.tag == id){
                list.push(this)
            }
         })
         onlineImage.pushData(list);
       }
    })
    OnlineImage.prototype = {
        ...
        /* 初始化第一次的标签 */
        initTag: function () {
            $.post(location.protocol+"//"+location.host+'/index.php/utils/ueditor/initimgtag',{},function(d){
               var html = "";
               html+="<a href='javascript:'data-tagimg-id='全部图片' class='checkimg'>全部图片("+d.data.total_num+")</a>";
               $.each(d.data.tags,function(){
                   html+=" | <a href='javascript:' data-tagimg-id='"+this.id+"'>"+this.tag+"("+this.num+")</a>"
               })
               $('#imageListTag').html(html);
            },'json')  
        },
        /* 重置界面 */
        reset: function() {
                    ...
                    'onsuccess': function (r) {
                        try {
                            ...
                            if (json.state == 'SUCCESS') {
                                ...
                                window.onlineimagelist = json.list;
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.setAttribute('_cover_id', urlPrefix + list[i].cover_id);
            ...
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    ...
                    list.push({
                        src: src,
                        _src: src,
                        cover_id:img.getAttribute('_cover_id'),
    ...
    /* 在线图片 */
    function OnlineImage1(target) {
        this.container = utils.isString(target) ? document.getElementById(target) : target;
        this.init();
    }
    /****监听图片分组**************/
    $('body').on('click','[data-tagimg1-id]',function(){
        $('[data-tagimg1-id]').removeClass('checkimg');
        $(this).addClass('checkimg');
        onlineImage1 = onlineImage1 || new OnlineImage1('imageList1');
        onlineImage1.initContainer();
        onlineImage1.initEvents();
        var list = [];
        var id = $(this).attr('data-tagimg1-id');
        if(id =="全部图片"){
            onlineImage1.pushData(onlineimagelist1);
        }else{
         $.each(onlineimagelist1,function(){
            if(this.tag == id){
                list.push(this)
            }
         })
         onlineImage1.pushData(list);
       }
    })
    OnlineImage1.prototype = {
        init: function () {
            this.reset();
            this.initEvents();
        },
        ...
        initEvents: function () {
            ...
            /* 滚动拉取图片 */
            domUtils.on($G('imageList1'), 'scroll', function(e){
                var panel = this;
                if (panel.scrollHeight - (panel.offsetHeight + panel.scrollTop) < 10) {
                    _this.getImageData();
                }
        ...
        /* 初始化第一次的标签 */
        initTag: function () {
            $.post(location.protocol+"//"+location.host+'/index.php/utils/ueditor/initimgtag/merchant_id/pt',{},function(d){
               var html = "";
               html+="<a href='javascript:'data-tagimg1-id='全部图片' class='checkimg'>全部图片("+d.data.total_num+")</a>";
               $.each(d.data.tags,function(){
                   html+=" | <a href='javascript:' data-tagimg1-id='"+this.id+"'>"+this.tag+"("+this.num+")</a>"
               })
               $('#imageListTag1').html(html);
            },'json')  
        },
        /* 重置界面 */
        reset: function() {
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                var url = editor.getActionUrl(editor.getOpt('imageManagerActionName')),
                    isJsonp = utils.isCrossDomainUrl(url);
                ajax.request(url+"&merchant_id=pt", {
                    'timeout': 100000,
                    'dataType': isJsonp ? 'jsonp':'',
                    'data': utils.extend({
                            start: this.listIndex,
                    ...
                    'onsuccess': function (r) {
                        try {
                            ...
                            if (json.state == 'SUCCESS') {
                                _this.pushData(json.list);
                                window.onlineimagelist1 = json.list;
                                _this.listIndex = parseInt(json.start) + parseInt(json.list.length);
                                if(_this.listIndex >= json.total) {
                                    _this.listEnd = true;
                                }
