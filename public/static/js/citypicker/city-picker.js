(function(factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function() {
            var city_flag=true;
            if(typeof flag_city != "undefined" && flag_city==true){
                 city_flag=false;   
            }
            var p = this.getPosition(),
                ...
                (this.includeDem('city') ? '<a data-count="city">城市</a>' : '') +
                 (city_flag?(this.includeDem('district') ? '<a data-count="district">区/县名</a>' : ''):'') +
                 (city_flag?(this.includeDem('county') ? '<a data-count="county">镇名</a>' : ''):'') +
//                 (city_flag?'':'<a data-count="cleans" id="cleans">清除</a>') +
        ...
        getList: function(data, type) {
            ...
            list.push('</dd></dl>');
            if(typeof flag_city != "undefined"){
                if(type=='district' && flag_city==true){
                    $this.close(true);
                    this.tab(COUNTY);
                  return ;  
                }
        ...
        tab: function(type) {
            ...
            if(typeof flag_city != "undefined" && flag_city==true){
                if(type=='cleans' ){
                    this.$element.val('cesgu ')
//                    this.$element.trigger('cp:updated');
                    cleans_city();//清除城市选择，写在页面的方法
                    $this.close(true);
                }
            }
