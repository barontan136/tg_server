...
class Wechat {
    ...
    const EVENT_USER_VIEW_CARD = 'user_view_card';        //卡券 - 用户进入卡券事件
    const EVENT_CARD_USER_GET = 'user_get_card';        //卡券 - 用户领取卡券
    const EVENT_CARD_USER_DEL = 'user_del_card';        //卡券 - 用户删除卡券
    const EVENT_SUBMIT_MEMBERCARD_USER_INFO = 'submit_membercard_user_info';        //卡券 - 用户激活会员卡
    const API_URL_PREFIX = 'https://api.weixin.qq.com/cgi-bin';
    const POI_GETPOILIST = '/poi/getpoilist?'; //查询门店列表
    ...
    const USER_GETBLACKLIST = '/tags/members/getblacklist?';
    const USER_BATCHBLACKLIST = '/tags/members/batchblacklist?';
    const USER_BATCHUNBLACKLIST = '/tags/members/batchunblacklist?';
    ...
    const OAUTH_TOKEN_COMPONENT_URL = '/sns/oauth2/component/access_token?';
    ...
    ///第三方开放平台
    const API_QUERY_AUTH = '/cgi-bin/component/api_query_auth?'; //使用授权码换取公众号的接口调用凭据和授权信息
    const API_AUTHORIZER_TOKEN = '/cgi-bin/component/api_authorizer_token?'; //获取（刷新）授权公众号的接口调用凭据（令牌）
    const API_GET_AUTHORIZER_INFO = '/cgi-bin/component/api_get_authorizer_info?'; //获取授权方的公众号帐号基本信息
    const MP_BASE_URL_PREFIX = 'https://mp.weixin.qq.com'; //公众号授权调整地址
    const API_COMPONENT_TOKEN = "/cgi-bin/component/api_component_token"; //获取第三方平台component_access_token
    const API_CREATE_PREAUTHCODE = "/cgi-bin/component/api_create_preauthcode?"; //获取预授权码pre_auth_code
    const OAUTH_COMPONENT_LOGIN_URL = '/cgi-bin/componentloginpage?'; //公账号授权页面
    const UPLOAD_CARD_AGENT_QUALIFICATION = "/cgi-bin/component/upload_card_agent_qualification?"; //申请成为母商户接口(强制授权)
    const CHECK_CARD_AGENT_QUALIFICATION = "/cgi-bin/component/check_card_agent_qualification?"; //母商户资质审核查询接口(强制授权)
    const UPLOAD_CARD_MERCHANT_QUALIFICATION = "/cgi-bin/component/upload_card_merchant_qualification?"; //子商户资质提交(强制授权)
    const CHECK_CARD_MERCHANT_QUALIFICATION = "/cgi-bin/component/check_card_merchant_qualification?"; //子商户资质审核查询接口(强制授权)
    const GET_CARD_MERCHANT = "/cgi-bin/component/get_card_merchant?"; //拉取单个子商户信息接口(强制授权)
    const BATCHGET_CARD_MERCHANT = "/cgi-bin/component/batchget_card_merchant?"; //拉取子商户列表接口(强制授权)
    //卡券相关
    ...
    const CARD_MEMBERCARD_USERINFOGET = '/card/membercard/userinfo/get?';      //拉取会员数据
    const CARD_MEMBERCARD_ACTIVATEUSERFORM = '/card/membercard/activateuserform/set?';      //一键激活会员卡
    const CARD_MEMBERCARD_UPDATEUSER = '/card/membercard/updateuser?';    //更新会员卡
    const CARD_PAYGIFTMEMBERCARDGET = '/card/paygiftmembercard/get?';    //查询商户号是否支持支付即会员
    const CARD_PAYGIFTMEMBERCARDADD = '/card/paygiftmembercard/add?';    //支付即会员
    const CARD_PAYGIFTMEMBERCARDDELETE = '/card/paygiftmembercard/delete?';    //支付即会员
    ...
    const CARD_GETAPPLYPROTOCOL = '/card/getapplyprotocol?';     //卡券开放类目查询接口
    const CARD_SUBMERCHANT_SUBMIT = '/card/submerchant/submit?';     //提交子商户
    const CARD_SUBMERCHANTGET = '/card/submerchant/get?';     //拉取单个子商户信息接口(代制模式)
    const CARD_SUBMERCHANTBATCHGET = '/card/submerchant/batchget?';     //批量拉取子商户信息接口(代制模式)
    ...
    public $component_encodingAesKey;
     ...
     * For weixin server validation
     */
    private function checkComponentSignature($str = '') {
        $signature = isset($_GET["signature"]) ? $_GET["signature"] : '';
        $signature = isset($_GET["msg_signature"]) ? $_GET["msg_signature"] : $signature; //如果存在加密验证则用加密验证段
        $timestamp = isset($_GET["timestamp"]) ? $_GET["timestamp"] : '';
        $nonce = isset($_GET["nonce"]) ? $_GET["nonce"] : '';
     ...
     * @param bool $return 是否返回
     */
    public function validComponent($return = true) {
        $encryptStr = "";
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            $postStr = file_get_contents("php://input");
            $array = (array) simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
     ...
     * @param bool $return 是否返回信息而不抛出到浏览器 默认:否
     */
    public function replyComponent($msg = array(), $return = false) {
        if (empty($msg)) {
            if (empty($this->_msg))   //防止不先设置回复内容，直接调用reply方法导致异常
                return false;
            $msg = $this->_msg;
...
//                writerLog($sContent);
     ...
     * @param string $token 手动指定access_token，非必要情况不建议用
     */
    public function getComponentAccessToken($component_appid = '', $component_appsecret = '', $component_access_token = '') {
        ...
        $m = db('token');
        while (true) {
            $map['name'] = 'component_access_token';
            $data = $m->where($map)->find();
            if ($data) {
                break;
            }
        ...
        if ($data['expire_time'] + 7000 < $now) {
            ...
            $mp['name'] = "component_verify_ticket";
            $t = $m->where($mp)->find();
            $post['component_appid'] = $this->component_appid;
            $post['component_appsecret'] = $this->component_appsecret;
            $post['component_verify_ticket'] = $t['value'];
            $result = $this->http_post(self::API_BASE_URL_PREFIX . self::API_COMPONENT_TOKEN, $this->json_encode($post));
            if ($result) {
                $json = json_decode($result, true);
                if (!$json || !empty($json['errcode'])) {
                    $this->errCode = $json['errcode'];
                    ...
                    if ($this->component_access_token) {
                        $data['expire_time'] = $now;
                        $data['value'] = $this->component_access_token;
                        $m->update($data);
                    }
                    return $this->component_access_token;
                }
            ...
            $this->component_access_token = $data['value'];
            return $this->component_access_token;
        }
        return false;
    }
    ...
    public function getJsTicket($appid = '', $jsapi_ticket = '') {
        ...
        if ($data['expire_time'] + 7000 < $now) {
            ...
            if ($result) {
                ...
                $data['expire_time'] = $now;
                $data['value'] = $this->jsapi_ticket;
                $m->update($data);
                return $this->jsapi_ticket;
            }
        } else {
            $ticket = $data['value'];
            $this->jsapi_ticket = $data['value'];
            return $ticket;
        }
        return false;
    }
     ...
     * @param string $jsapi_ticket 手动指定jsapi_ticket，非必要情况不建议用
     */
    public function getJsTicketComponent($appid = '', $jsapi_ticket = '') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        if (!$appid)
            return false;
        ...
        $m = db('component_appid');
        $map['authorizer_appid'] = $appid;
        $data = $m->where($map)->find();
        $now = time();
        if ($data['ticket_js_expires_in'] + 7000 < $now) {
            ...
            if ($result) {
                ...
                $this->jsapi_ticket = $json['ticket'];
                $data['ticket_js_expires_in'] = $now;
                $data['ticket_js'] = $this->jsapi_ticket;
                $m->update($data);
                return $this->jsapi_ticket;
            }
        } else {
            $ticket = $data['ticket_js'];
            $this->jsapi_ticket = $data['ticket_js'];
            return $ticket;
        }
        return false;
    }
     ...
     * @return array|bool 返回签名字串
     */
    public function getJsSignComponent($url, $timestamp = 0, $noncestr = '', $appid = '') {
        if (empty($appid)) {
            return false;
        }
        if (!$this->jsapi_ticket && !$this->getJsTicketComponent($appid) || !$url)
            return false;
        if (!$timestamp)
            $timestamp = time();
        if (!$noncestr)
     ...
     * 获取微信卡券api_ticket
     * @param string $appid 用于多个appid时使用,可空
     * @param string $card_ticket $card_ticket，非必要情况不建议用
     */
    public function getJsCardTicket($appid = '', $card_ticket = '') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        if (!$appid)
            $appid = $this->appid;
        ...
        if ($data['expire_time'] + 7000 < $now) {
            ...
            if ($result) {
                ...
                $data['expire_time'] = $now;
                $data['value'] = $this->card_ticket;
                $m->update($data);
            }
            return $this->card_ticket;
        } else {
            $this->card_ticket = $data['value'];
            return $this->card_ticket;
        }
        return false;
    }
     ...
     * @param string $card_ticket $card_ticket，非必要情况不建议用
     */
    public function getJsCardTicketComponent($appid = '', $card_ticket = '') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        if (!$appid)
            return false;
        ...
        $data = $m->where($map)->find();
        $now = time();
        if ($data['ticket_card_expires_in'] + 7000 < $now) {
            $result = $this->http_get(self::API_URL_PREFIX . self::GET_TICKET_URL . 'access_token=' . $this->access_token . '&type=wx_card');
            if ($result) {
                $json = json_decode($result, true);
                if (!$json || !empty($json['errcode'])) {
                ...
                $this->card_ticket = $json['ticket'];
                $data['ticket_card_expires_in'] = $now;
                $data['ticket_card'] = $this->card_ticket;
                $m->update($data);
            }
            return $this->card_ticket;
        } else {
            $this->card_ticket = $data['ticket_card'];
            return $this->card_ticket;
        }
        return false;
    }
     ...
     * )
     */
    public function getForeverList($type, $offset=0, $count=20) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data = array(
            'type' => $type,
     ...
     * 创建二维码ticket
     * 默认临时整数型
     * @param int|string $scene_id 自定义追踪id
     * @param int $type 0:临时的整型参数值二维码；1:永久的整型参数值二维码；2:永久的字符串参数值二维码;3.临时的字符串参数值
     * 永久型expire(时间)参数无效
    ...
    public function getQRCode($scene_id, $type = 0, $expire = 2592000) {
        ...
        $action_info = array('scene_id' => $scene_id);
        switch ($type) {
            case 0:
                break;
            case 1:
            ...
            case 2:
                ...
                $action_info = array('scene_str' => $scene_id);
                break;
            case 3:
        ...
        $data = array(
            ...
            'action_info' => array('scene' => $action_info)
        );
        if ($type == 1 || $type == 2) {
    ...
    /**
     * 下载二维码图片
     * @param type $ticket 传入由getQRCode方法生成的ticket参数
     * @param string $name 文件名称(如果为空则系统随机生成)
     * @param string $local 文件下载地址 格式"test/test",为空默认为public/文件夹
     * @return string url 返回文件下载地址
     */
    public function downloadQRcode($ticket, $name, $local) {
        if (empty($ticket))
            return false;
        if (empty($name)) {
            $name = time() . rand(1, 9) . rand(1, 9) . rand(1, 9) . rand(1, 9) . rand(1, 9);
        }
        if (empty($local)) {
            $local = "WechatQrcode";
        }
        define('__QRCODE_PATH__', './Uploads/' . $local);
        if (!file_exists(__QRCODE_PATH__)) {
            @mkdir(__QRCODE_PATH__, 0777, true);
        }
        $url = $this->getQRUrl($ticket);
        $imageInfo = $this->downloadImageFromWeiXin($url);
        $filename = __QRCODE_PATH__ . "/" . iconv("UTF-8", "gb2312", $name) . rand(1, 9) . rand(1, 9) . rand(1, 9) . rand(1, 9) . rand(1, 9) . ".png";
     ...
     * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
     */
    public function getComponentUserInfo($appid, $openid) {
     ...
     * 注意：unionid字段 只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
     */
    public function getUsersInfo($openids,$lang = 'zh_CN') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        foreach (array_unique($openids) as $openid) {
            $data['user_list'][] = array('openid' => $openid, 'lang' => $lang);
        }
        $result = $this->http_post(self::API_URL_PREFIX . self::USERS_INFO_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getBacklist($begin_openid) {
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self:: USER_GETBLACKLIST. "access_token={$this->access_token}", self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @param string $openid
     */
    public function batchBlacklist($openids) {
         if (!$this->access_token && !$this->checkAuth()) {
            return false;
        }
        $data = ['openid_list' => $openids];
        $result = $this->http_post(self::API_URL_PREFIX . self:: USER_BATCHBLACKLIST. "access_token={$this->access_token}", self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @param string $openid
     */
    public function batchunBlacklist($openids) {
         if (!$this->access_token && !$this->checkAuth()) {
            return false;
        }
        $data = ['openid_list' => $openids];
        $result = $this->http_post(self::API_URL_PREFIX . self::USER_BATCHUNBLACKLIST . "access_token={$this->access_token}", self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @param unknown $next_openid
     */
    public function getGroupUserList($tagid='',$next_openid = '') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data=array(
            'tagid'=>$tagid,
    ...
    public function getUserGroup($openid) {
        ...
        if ($result) {
            ...
            return isset($json['tagid_list'])?$json['tagid_list']:false;
        }
        return false;
    }
     ...
     * @return boolean|array
     */
    public function batchUpdateTagsMembers($groupid, $openid_list) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data = array(
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::TAGS_MEMBER_BATCHTAGGING_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
    public function batchunUpdateTagsMembers($groupid, $openid_list) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data = array(
            'openid_list' => $openid_list,
        ...
        $result = $this->http_post(self::API_URL_PREFIX . self::TAGS_MEMBER_BATCHUNTAGGING_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     */
    public function deleteTag($groupid) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $data = array(
     ...
     * @param type $scope
     */
    public function wxOAuth2($scope = null) {
        ...
        if ($code) {
            ...
            !$json && alert('获取用户授权失败，请重新确认');
            $openid = $json['openid'];
//            $userinfo = $this->getUserInfo($openid);
            $userinfo = $this->getOauthUserinfo($this->user_token, $openid);
            if ($userinfo) {
                $wxuser = ['openid' => $openid];
                writerLog('wxOauth2=>user_info:' . $userinfo['nickname'] ?? 'empty');
...
//                        'subscribe' => intval($userinfo['subscribe']??0),
                        'province' => $userinfo['province'],
                        'city' => $userinfo['city'],
                        'country' => $userinfo['country'],
//                        'tagid_list'=>  array2string($userinfo['tagid_list']??0),
            ...
            writerLog($oauth_url);
            echo "<script>location.replace('".$oauth_url."')</script>";
     ...
     * 第三方开放平台网页授权
     */
    public function wxOAuth2Component($appid, $scope = null) {
        if (empty($appid)) {
            die('获取用户授权失败，appid不能为空');
        }
        if (!isset($scope) && empty($scope)) {
            $scope = 'snsapi_userinfo';
        } else {
        ...
        if ($code) {
            $json = $this->getOauthAccessTokenComponent($appid);
            if (!$json) {
                die('获取用户授权失败，请重新确认');
            }
            $openid = $json['openid'];
            $userinfo = $this->getComponentUserInfo($appid,$openid);
            ...
            $oauth_url = getWeObject($appid)->getOauthComponentRedirect($url, $appid, "wxbase", $scope);
            echo "<script>location.replace('".$oauth_url."')</script>";
            exit;
        }
    }
     ...
     * @return string
     */
    public function getOauthComponentRedirect($callback, $appid, $state = '', $scope) {
        if (empty($appid)) {
            die('授权地址错误,请提供appid');
        }
        return self::OAUTH_PREFIX . self::OAUTH_AUTHORIZE_URL . 'appid=' . $appid . '&redirect_uri=' . urlencode($callback) . '&response_type=code&scope=' . $scope . '&state=' . $state . '&component_appid=' . $this->component_appid . '#wechat_redirect';
    }
     ...
     * @return string
     */
    public function getComponentLoginPage($callback) {
        return self::MP_BASE_URL_PREFIX . self::OAUTH_COMPONENT_LOGIN_URL . 'component_appid=' . $this->component_appid . '&pre_auth_code=' . $this->api_create_preauthcode() . '&redirect_uri=' . urlencode($callback);
    }
    ...
    public function api_query_auth($authorization_code) {
        ...
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::API_QUERY_AUTH . 'component_access_token=' . $this->getComponentAccessToken(), $this->json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getAuthorizerAccessToken($authorizer_appid) {
        ...
        $component = $m->where($map)->find();
        if ($component) {
            $time = time();
            if ($component['expires_in'] + 7000 < $time) {
                $data['authorizer_appid'] = $authorizer_appid;
                $data['authorizer_refresh_token'] = $component['authorizer_refresh_token'];
                $data['component_appid'] = $this->component_appid;
                $result = $this->http_post(self::API_BASE_URL_PREFIX . self::API_AUTHORIZER_TOKEN . 'component_access_token=' . $this->getComponentAccessToken(), $this->json_encode($data));
                if ($result) {
                    $json = json_decode($result, true);
                    if (!$json || !empty($json['errcode'])) {
                        $this->errCode = $json['errcode'];
                    ...
                    $component['authorizer_access_token'] = $json['authorizer_access_token'];
                    $component['expires_in'] = $time;
                    $m->update($component);
                    return $component['authorizer_access_token'];
                }
                return false;
            } else {
                return $component['authorizer_access_token'];
    ...
    public function api_get_authorizer_info($authorizer_appid) {
        ...
        $data['component_appid'] = $this->component_appid;
        $data['authorizer_appid'] = $authorizer_appid;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::API_GET_AUTHORIZER_INFO . 'component_access_token=' . $this->getComponentAccessToken(), $this->json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getOauthAccessToken($code='') {
        ...
        $result = $this->http_get(
            self::API_BASE_URL_PREFIX . self::OAUTH_TOKEN_URL . 'appid=' . $this->appid . '&secret=' . $this->appsecret . '&code=' . $code . '&grant_type=authorization_code');
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array {access_token,expires_in,refresh_token,openid,scope}
     */
    public function getOauthAccessTokenComponent($appid,$code='') {
        if(empty($code)){
          $code = isset($_GET['code']) ? $_GET['code'] : '';
        }
        if (!$code)
            return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::OAUTH_TOKEN_COMPONENT_URL . 'appid=' . $appid . '&code=' . $code . '&grant_type=authorization_code&component_appid=' . $this->component_appid . '&component_access_token=' . $this->getComponentAccessToken());
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getpoilist($begin = 0, $limit = 50) {
        ...
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::POI_GETPOILIST . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getTemplateList() {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_get(self::API_URL_PREFIX . self::TEMPLATE_GET_ALL_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 模板消息 删除消息模板
     * 成功返回消息模板的调用id
     * @param string $long_tpl_id 模板库中长id
     * @return boolean|string
     */
    public function delTemplateMessage($long_tpl_id) {
        $data = array('template_id' => $long_tpl_id);
        if (!$this->access_token && !$this->checkAuth())
            return false;
     ...
     * 卡券开放类目查询接口
     */
    public function card_getapplyprotocol() {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::CARD_GETAPPLYPROTOCOL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function api_create_preauthcode() {
        ...
        $now = time();
        if ($data['expire_time'] + 600 < $now) {
            $post['component_appid'] = $this->component_appid;
            $result = $this->http_post(self::API_BASE_URL_PREFIX . self::API_CREATE_PREAUTHCODE . 'component_access_token=' . $this->component_access_token, self::json_encode($post));
            if ($result) {
                $json = json_decode($result, true);
                if (!$json || !empty($json['errcode'])) {
                    $this->errCode = $json['errcode'];
     ...
     * 母商户资质申请接口
     */
    public function upload_card_agent_qualification($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::UPLOAD_CARD_AGENT_QUALIFICATION . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function check_card_agent_qualification() {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::CHECK_CARD_AGENT_QUALIFICATION . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 子商户资质提交
     */
    public function upload_card_merchant_qualification($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::UPLOAD_CARD_MERCHANT_QUALIFICATION . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 子商户资质审核查询
     */
    public function check_card_merchant_qualification($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CHECK_CARD_MERCHANT_QUALIFICATION . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 拉取单个子商户信息接口(强制授权)
     */
    public function get_card_merchant($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::GET_CARD_MERCHANT . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 拉取子商户列表接口(强制授权)
     * 一次调用最多拉取20个子商户的信息，可以通过多次拉去满足不同的查询需求。
     */
    public function batchget_card_merchant($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::BATCHGET_CARD_MERCHANT . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array|boolean 返回子商户数据
     */
    public function card_submerchant_submit($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_SUBMERCHANT_SUBMIT . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array|boolean 返回子商户数据
     */
    public function card_submerchantget($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_SUBMERCHANTGET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array|boolean 返回子商户数据
     */
    public function card_submerchantbatchget($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_SUBMERCHANTBATCHGET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|string
     */
    public function createCardQrcode($card_id, $outer_str = '0', $code = '', $openid = '', $expire_seconds = 0, $is_unique_code = false, $balance = '') {
        $card = array(
            'card_id' => $card_id
        );
        $data = array(
     ...
     * @return boolean
     */
    public function memberCardUserinfoGet($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_USERINFOGET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean
     */
    public function activateUserFormMemberCard($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_ACTIVATEUSERFORM . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 查询商户号是否支持支付即会员
     */
    public function paygiftmembercardget($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_PAYGIFTMEMBERCARDGET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 查询商户号是否支持支付即会员
     */
    public function paygiftmembercardadd($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_PAYGIFTMEMBERCARDADD . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 删除支付即会员
     */
    public function paygiftmembercarddelete($data) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_PAYGIFTMEMBERCARDDELETE . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean
     */
    public function getDataByUrl($url, $data, $flag = true) {
        if ($flag) {
   ...
   /**
    ...
    public function setError() {
        if (isset($this->eCode[$this->errCode])) {
            $this->errMsg = $this->eCode[$this->errCode];
        } 
    }
